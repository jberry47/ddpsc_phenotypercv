...
inline vector<double> get_shapes(vector<Point> cc,Mat mask){
    //-- Get measurements
...
inline Mat get_color(Mat img,Mat mask){
	Mat composite;
...
inline Mat get_nir(Mat img,Mat mask){
    Mat hist;
	int dims = 1;
	int histSize = 255;
	float hranges[] = { 0, 255 };
...
inline vector<Point> keep_roi(Mat img,Point tl, Point br, Mat &mask){
	//-- Get contours of mask
	vector<vector<Point> > contours;
	vector<Vec4i> hierarchy;
    findContours( img, contours, hierarchy, RETR_TREE, CHAIN_APPROX_SIMPLE, Point(0, 0) );
...
inline void write_shapes(vector<double> shapes_data, string iname, string fname){
	ofstream shape_file;
	shape_file.open(fname.c_str(),ios_base::app);
	shape_file << iname << " ";
