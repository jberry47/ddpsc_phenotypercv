...
inline void trainSVM(Mat img, Mat mask, string fname){
	Mat imageR, maskR;
...
inline float calcSVM(Vec3b pixel,Ptr<SVM> svm){
...
inline Mat predictSVM(Mat img, string fname){
	Mat imageR;
...
inline float calcBC(Vec3b pixel,Ptr<NormalBayesClassifier> bc){
	float arr[3] = {pixel[0],pixel[1],pixel[2]};
	Mat arrM(1,3,CV_32F,arr);
    float back = bc->predict(arrM);
    return(back);
};

inline Mat predictBC(Mat img, string fname){
	Mat imageR;
